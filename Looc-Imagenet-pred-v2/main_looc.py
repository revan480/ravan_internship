...
def parse_args():
    ...
    parser.add_argument("--pred-mode", type=str, default="angle",
                        choices=["angle", "combo"],
                        help="prediction mode: 'angle'=rotation angle (3 classes), 'combo'=augmentation combination (4 classes)")
...
def main():
    ...
    # Map pred_mode to num_pred_classes
    num_pred_classes = 3 if args.pred_mode == "angle" else 4
    ...
    if args.lambda_pred > 0:
        print(f"  Aug prediction:   enabled (lambda={args.lambda_pred}, mode={args.pred_mode}, classes={num_pred_classes})")
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.lambda_pred > 0:
            epoch_str += f"  PredLoss: {pred_loss_avg:.4f}  AugAcc: {aug_acc_avg:.2f}%"
        print(epoch_str)
...
def train_one_epoch(train_loader, model, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        # Separate views from rotation label (loader always returns it as last element)
        views = [v.cuda(non_blocking=True) for v in images[:-1]]
        aug_labels = images[-1].cuda(non_blocking=True) if args.lambda_pred > 0 else None

        # Forward pass (loss computed inside model)
        loss, logits, labels, pred_loss_val, aug_acc_val = model(views, aug_labels)
        ...
        if args.lambda_pred > 0:
            pred_losses.update(pred_loss_val, views[0].size(0))
            aug_accs.update(aug_acc_val, views[0].size(0))

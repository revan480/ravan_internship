...
def parse_args():
    parser = argparse.ArgumentParser(description="Linear Evaluation for LooC / MoCo")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="load from LooC checkpoint (prefix: backbone_q) instead of MoCo (prefix: encoder_q)",
    )
    parser.add_argument(
        "--looc-plus",
        action="store_true",
        default=False,
        help="LooC++ mode: concatenate features from all projection heads' first layers",
    )
    parser.add_argument("--n-aug", type=int, default=2, help="number of atomic augmentations (for LooC++)")
...
class LooCPlusModel(nn.Module):
    """
    LooC++ evaluation model: backbone + first linear layer of each projection head.

    For each image:
      backbone → 2048-dim → each head's first layer + ReLU → concat → fc → logits

    Feature dimension = n_heads × 2048.
    """

    def __init__(self, backbone, head_first_layers, num_classes):
        """
        Args:
            backbone: ResNet-50 with fc=Identity (outputs 2048-dim), frozen
            head_first_layers: list of nn.Linear(2048, 2048), one per head, frozen
            num_classes: number of output classes
        """
        super().__init__()
        self.backbone = backbone
        self.head_first_layers = nn.ModuleList(head_first_layers)
        feat_dim = 2048 * len(head_first_layers)
    ...
    def forward(self, x):
        ...
        concat = torch.cat(hiddens, dim=1)  # (N, n_heads * 2048)
...
def load_moco_weights(model, pretrained_path):
    ...
    MoCo checkpoint keys: encoder_q.layer1.0.conv1.weight, etc.
    Strips 'encoder_q.' prefix and skips fc (projection head).
    """
...
def load_looc_weights(model, pretrained_path):
    """Load LooC pre-trained weights into the model backbone.

    LooC checkpoint keys: backbone_q.layer1.0.conv1.weight, etc.
    Strips 'backbone_q.' prefix and skips fc (Identity in LooC).
    """
    ...
    for k, v in state_dict.items():
        ...
        # Skip fc (Identity in LooC, but standard ResNet has fc)
        if new_key.startswith("fc."):
            skipped.append(new_key)
            continue
        new_state_dict[new_key] = v
...
def build_looc_plus_model(args, num_classes):
    """Build a LooCPlusModel from a LooC checkpoint.

    Loads the backbone and each projection head's first linear layer,
    concatenates their ReLU'd outputs for a richer feature representation.
    """
    print(f"=> Loading LooC checkpoint for LooC++ mode: {args.pretrained}")
    checkpoint = torch.load(args.pretrained, map_location="cpu")
    state_dict = checkpoint["state_dict"]

    # Count heads by looking for heads_q.{i}.0.weight keys
    n_heads = 0
    ...
    backbone.fc = nn.Identity()  # output 2048-dim, not 1000
    ...
    # Create model
    model = LooCPlusModel(backbone, head_first_layers, num_classes)
    print(f"  Backbone + {n_heads} head layers loaded and frozen")
    print(f"  Trainable: fc layer ({feat_dim} -> {num_classes})")
    print("=> LooC++ model built successfully")
    return model
...
def main():
    ...
    eval_mode = "LooC++" if args.looc_plus else ("LooC" if args.looc_backbone else "MoCo")
    print(f"  Checkpoint type:{eval_mode}")
    ...
    # Build model
    if args.looc_plus:
        # LooC++ mode: backbone + concatenated head features
        model = build_looc_plus_model(args, num_classes)
    else:
        # Standard mode: backbone only

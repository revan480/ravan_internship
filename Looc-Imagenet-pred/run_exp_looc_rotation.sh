#!/bin/bash
# Experiment: LooC with rotation only (n_aug=1, aug_types=rotation)
set -e

DATA="./imagenet100"
SAVE_DIR="./checkpoints/exp_looc_rotation"
LOG="exp_looc_rotation.log"
CSV="exp_looc_rotation_results.csv"
ARCH="resnet50"
EPOCHS_PRETRAIN=500
EPOCHS_LINCLS=200

echo "Starting exp_looc_rotation (LooC, aug_types=rotation)" | tee ${LOG}
...
# Append to CSV
echo "exp_looc_rotation,${ARCH},${EPOCHS_PRETRAIN},${EPOCHS_LINCLS},256,16384,0.999,0.2,128,0.03,30.0,rotation,${OBJ_ACC1},${OBJ_ACC5},${ROT_ACC1},${ROT_ACC4}" >> ${CSV}

echo "" | tee -a ${LOG}
echo "exp_looc_rotation COMPLETE" | tee -a ${LOG}

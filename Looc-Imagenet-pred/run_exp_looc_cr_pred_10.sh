#!/bin/bash
# Experiment: LooC color+rotation + augmentation prediction classifier (lambda=1.0)
set -e

DATA="./imagenet100"
SAVE_DIR="./checkpoints/exp_looc_cr_pred_10"
LOG="exp_looc_cr_pred_10.log"
CSV="exp_looc_cr_pred_10_results.csv"
ARCH="resnet50"
EPOCHS_PRETRAIN=500
EPOCHS_LINCLS=200

echo "Starting exp_looc_cr_pred_10 (LooC + pred, lambda=1.0)" | tee ${LOG}
...
echo "" | tee -a ${LOG}
echo ">>> Step 1: LooC Pre-Training (${EPOCHS_PRETRAIN} epochs, lambda_pred=1.0)" | tee -a ${LOG}
python main_looc.py \
    --data ${DATA} \
    --arch ${ARCH} \
...
# Append to CSV
echo "exp_looc_cr_pred_10,${ARCH},${EPOCHS_PRETRAIN},${EPOCHS_LINCLS},256,16384,0.999,0.2,128,0.03,30.0,rotation+color,1.0,${OBJ_ACC1},${OBJ_ACC5},${ROT_ACC1},${ROT_ACC4}" >> ${CSV}

echo "" | tee -a ${LOG}
echo "exp_looc_cr_pred_10 COMPLETE" | tee -a ${LOG}

...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["invariance", "sensitivity", "invariance_sensitivity"],
...
def main():
    ...
    if args.baseline:
        ...
        if args.condition in ("sensitivity", "invariance_sensitivity"):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        elif args.condition == "invariance_sensitivity":
            loss_avg, pred_loss_avg, pred_acc_avg = train_one_epoch_invariance_sensitivity(
                train_loader, model, criterion, optimizer, epoch, args
            )
            print(
                f"Epoch [{epoch + 1}/{args.epochs}]  "
...
def train_one_epoch_invariance_sensitivity(train_loader, model, criterion, optimizer, epoch, args):
    """Train for one epoch — invariance_sensitivity condition.

    Each view has its own independent transformation label.
    Prediction loss is averaged across per-view cross-entropies.
    """
    batch_time = AverageMeter("Time", ":.3f")
    data_time = AverageMeter("Data", ":.3f")
    losses = AverageMeter("Loss", ":.4f")
    ...
    for i, (data, _) in enumerate(train_loader):
        ...
        # data = [view1_batch, view2_batch, label1_batch, label2_batch]
        view1 = data[0].cuda(non_blocking=True)
        view2 = data[1].cuda(non_blocking=True)
        label1 = data[2].cuda(non_blocking=True)
        label2 = data[3].cuda(non_blocking=True)
        ...
        # Per-view prediction loss (independent labels)
        pred_loss = (F.cross_entropy(aug_logits1, label1) + F.cross_entropy(aug_logits2, label2)) / 2

        # Total loss
        loss = contrastive_loss + args.pred_lambda * pred_loss

        # Per-view prediction accuracy
        acc1 = (aug_logits1.argmax(1) == label1).float().mean().item() * 100
        acc2 = (aug_logits2.argmax(1) == label2).float().mean().item() * 100
        pred_acc = (acc1 + acc2) / 2

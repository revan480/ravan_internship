...
from simclr.builder import SimCLRPred
from simclr.loader import SimCLRPredTransform
from simclr.loss import NTXentLoss


def parse_args():
    parser = argparse.ArgumentParser(description="SimCLR + Aug Prediction Pre-Training (Single GPU)")
...
def main():
    ...
    print("=" * 70)
    print("SimCLR + Augmentation Prediction Pre-Training Configuration")
    print("=" * 70)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loss_avg, pred_loss_avg, pred_acc_avg = train_one_epoch(
...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (data, _) in enumerate(train_loader):
        ...
        z1, aug_logits1 = model(view1)  # (N, dim), (N, num_pred_classes)
        z2, aug_logits2 = model(view2)  # (N, dim), (N, num_pred_classes)

        # Contrastive loss
        z = torch.cat([z1, z2], dim=0)  # (2N, dim)
        contrastive_loss = criterion(z)

        # Augmentation prediction loss (both views predict same label)

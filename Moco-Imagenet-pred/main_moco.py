...
def parse_args():
    ...
    parser.add_argument("--lambda-pred", type=float, default=0.0,
                        help="weight for augmentation prediction loss (0=disabled)")
    parser.add_argument("--num-aug-classes", type=int, default=4,
                        help="number of augmentation combination classes")
...
def main():
    ...
    if args.lambda_pred > 0:
        print(f"  Aug prediction:   lambda={args.lambda_pred}, classes={args.num_aug_classes}")
    else:
        print(f"  Aug prediction:   disabled")
    ...
    model = MoCo(
        ...
        num_aug_classes=args.num_aug_classes,
    )
    model.cuda()
    ...
    if args.lambda_pred > 0:
        # Augmentation prediction mode: separate query/key transforms.
        # Query transform tracks which augmentations were applied.
        # Key transform is standard (same as build_augmentation with color+rotation).
        query_transform = AugPredQueryTransform(
            use_rotation=args.use_rotation,
            use_color=args.use_color,
            color_strength=args.color_strength,
        )
        key_transform = build_augmentation(args)
        train_dataset = MoCoAugPredDataset(traindir, query_transform, key_transform)
    else:
        # Standard MoCo mode (backward compatible)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loss_avg, acc_avg, pred_loss_avg, aug_acc_avg = train_one_epoch(
        ...
        if args.lambda_pred > 0:
            msg += f"  PredLoss: {pred_loss_avg:.4f}  AugAcc: {aug_acc_avg:.2f}%"
        print(msg)
...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, args):
    ...
    pred_losses = AverageMeter("PredLoss", ":.4f")
    aug_accs = AverageMeter("AugAcc", ":.2f")

    meters = [batch_time, data_time, losses, accs]
    if args.lambda_pred > 0:
        meters.extend([pred_losses, aug_accs])
    ...
    for i, batch in enumerate(train_loader):
        ...
        if args.lambda_pred > 0:
            # MoCoAugPredDataset returns (query, key, aug_label)
            im_q, im_k, aug_labels = batch
            aug_labels = aug_labels.cuda(non_blocking=True)
        else:
            # Standard TwoCropsTransform returns ([view1, view2], class_label)
            images, _ = batch
        ...
        # Forward: returns (contrastive_logits, contrastive_labels, pred_loss, aug_acc)
        output, target, pred_loss, aug_acc = model(im_q=im_q, im_k=im_k, aug_labels=aug_labels)

        # Contrastive loss (same as standard MoCo)
        contrastive_loss = criterion(output, target)

        # Total loss = contrastive + lambda * prediction
        loss = contrastive_loss + args.lambda_pred * pred_loss

        acc = contrastive_accuracy(output, target)

        losses.update(loss.item(), im_q.size(0))
        accs.update(acc.item(), im_q.size(0))
        if args.lambda_pred > 0:
            pred_losses.update(pred_loss.item(), im_q.size(0))
            aug_accs.update(aug_acc, im_q.size(0))
    ...
    return losses.avg, accs.avg, pred_losses.avg, aug_accs.avg

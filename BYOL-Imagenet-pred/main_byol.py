...
from byol.builder import BYOLPred
from byol.loader import BYOLPredTransform, GaussianBlur
from byol.loss import BYOLLoss


def parse_args():
    parser = argparse.ArgumentParser(description="BYOL + Aug Prediction Pre-Training (Single GPU)")
    ...
    # BYOL
    parser.add_argument("--tau-base", type=float, default=0.996, help="base momentum for target EMA")
    parser.add_argument("--proj-hidden-dim", type=int, default=4096, help="projector hidden dimension")
    parser.add_argument("--proj-dim", type=int, default=256, help="projector output dimension")

    # Augmentation prediction
    parser.add_argument("--pred-lambda", type=float, default=0.5, help="weight for augmentation prediction loss")
    parser.add_argument("--num-pred-classes", type=int, default=4, help="number of augmentation combo classes")
...
def cosine_momentum_schedule(step, total_steps, base_tau=0.996):
    """Cosine momentum schedule from BYOL paper (Section 3.3).

    τ = 1 - (1 - τ_base) * (cos(π*k/K) + 1) / 2

    Starts at τ_base (0.996) and smoothly increases toward 1.0.
    """
    return 1 - (1 - base_tau) * (math.cos(math.pi * step / total_steps) + 1) / 2
...
def main():
    ...
    print("=" * 70)
    print("BYOL + Augmentation Prediction Pre-Training Configuration")
    print("=" * 70)
    ...
    print(f"  Proj dims:        2048 → {args.proj_hidden_dim} → {args.proj_dim}")
    print(f"  Pred lambda:      {args.pred_lambda}")
    print(f"  Pred classes:     {args.num_pred_classes}")
    ...
    model = BYOLPred(
        ...
        pred_hidden_dim=args.proj_hidden_dim,
        pred_dim=args.proj_dim,
        init_tau=args.tau_base,
        num_pred_classes=args.num_pred_classes,
    )
    model.cuda()

    # Loss and optimizer
    criterion = BYOLLoss().cuda()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loss_avg, pred_loss_avg, pred_acc_avg, tau_final = train_one_epoch(
...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (data, _) in enumerate(train_loader):
        ...
        # Forward: both views through online and target networks
        p1, p2, tz1, tz2, aug_logits1, aug_logits2 = model(view1, view2)

        # BYOL loss (symmetrized MSE)
        byol_loss = criterion(p1, p2, tz1, tz2)

        # Augmentation prediction loss (both views)
        ...
        # Total loss
        loss = byol_loss + args.pred_lambda * pred_loss
        ...
        # Backward (only updates online network + aug_classifier)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        model.tau = cosine_momentum_schedule(current_step, total_steps, args.tau_base)
    ...
    return losses.avg, pred_losses.avg, aug_accs.avg, model.tau

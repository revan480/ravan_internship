...
from byol.builder import BYOL
from byol.loader import BYOLTransform, GaussianBlur
from byol.loss import BYOLLoss


def parse_args():
    parser = argparse.ArgumentParser(description="BYOL Pre-Training (Single GPU)")
...
def main():
    ...
    print("=" * 70)
    print("BYOL Pre-Training Configuration")
    print("=" * 70)
    ...
    train_dataset = datasets.ImageFolder(
        traindir, BYOLTransform(augmentation)
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loss_avg, tau_final = train_one_epoch(
...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        p1, p2, tz1, tz2 = model(images[0], images[1])

        # BYOL loss (symmetrized MSE)
        loss = criterion(p1, p2, tz1, tz2)

        losses.update(loss.item(), images[0].size(0))

        # Backward (only updates online network)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

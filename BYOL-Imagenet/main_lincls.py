...
Usage:
    ...
    python main_lincls.py --data ./imagenet100 \
        --pretrained ./checkpoints/exp_simclr_color_rotation/checkpoint_0500.pth.tar --eval-rotation
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Linear Evaluation for SimCLR")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="evaluate rotation classification instead of object classes",
    )
...
def load_pretrained_weights(model, pretrained_path, args):
    """Load BYOL pre-trained weights into the model backbone.

    BYOL checkpoint keys: online_backbone.layer1.0.conv1.weight, online_projector.*, etc.
    Strips 'online_backbone.' prefix and skips all other keys.
    """
    ...
    # Extract online backbone weights, strip prefix, skip everything else
    new_state_dict = {}
    skipped = []
    for k, v in state_dict.items():
        if not k.startswith("online_backbone."):
            skipped.append(k)
            continue
        # Strip the "online_backbone." prefix
        new_key = k[len("online_backbone."):]
        # Skip fc (Identity in BYOL, but just in case)
...
def evaluate(test_loader, model, criterion, num_classes):

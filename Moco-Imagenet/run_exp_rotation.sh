#!/bin/bash
# Experiment: Rotation augmentation only (no color)
# --no-color --use-rotation
set -e

DATA="./imagenet100"
SAVE_DIR="./checkpoints/exp_rotation"
LOG="exp_rotation.log"

echo "Starting exp_rotation (rotation only, no color)" | tee ${LOG}
...
echo "" | tee -a ${LOG}
echo "exp_rotation COMPLETE" | tee -a ${LOG}
date | tee -a ${LOG}

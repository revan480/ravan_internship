...
Two evaluation modes:
  - Bird species classification (200 classes, default)
...
Usage:
    ...
    python main_lincls.py --data ~/Desktop/ravan/cub200_prepared \
        --pretrained ./checkpoints/exp_color/checkpoint_0200.pth.tar --eval-rotation
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="evaluate rotation classification instead of bird species",
    )
...
def main():
    ...
    # Auto-detect number of species classes from dataset
    traindir = os.path.join(args.data, "train")
    species_classes = len([d for d in os.listdir(traindir) if os.path.isdir(os.path.join(traindir, d))])
    num_classes = 4 if args.eval_rotation else species_classes
    task_name = f"Rotation Classification (4 classes)" if args.eval_rotation else f"Bird Species Classification ({species_classes} classes)"
    ...
    for epoch in range(args.epochs):
        ...
        test_loss, test_acc1, test_acc5 = evaluate(test_loader, model, criterion, num_classes)
        ...
        print(
            ...
            f"Test Loss: {test_loss:.4f}  Test Acc@1: {test_acc1:.2f}%  Test Acc@5: {test_acc5:.2f}%"
